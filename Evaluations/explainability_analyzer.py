#!/usr/bin/env python3
"""
ML Explainability Toolkit
-------------------------

I built this framework to help demystify ML models and make them more understandable
to stakeholders, auditors, and myself. It covers:

1. Comprehensive explainability assessment for different model types
2. Integration with XAI techniques like SHAP and LIME (lifesavers for black-box models!)
3. Visualization of feature importance and decision boundaries
4. Quantitative metrics to measure interpretability
5. Structured reporting that helps with regulatory compliance
6. Human-friendliness assessment of model explanations
7. Output formats optimized for different audiences

The toolkit handles various model types including:
...
Feel free to adapt this for your own needs! I've tried to make it modular enough
that you can use just the parts you need.
"""
...
# Don't worry if some are missing - we'll handle that gracefully
try:
    import pandas as pd
...
# Set up nice logging that goes to both console and file
def setup_logging(log_file='explainability.log', console_level=logging.INFO):
    """Set up dual logging - detailed logs in a file, important stuff in the console."""
    logger = logging.getLogger('explainability')
    logger.setLevel(logging.DEBUG)
    
    # Detailed logging to file for debugging and record-keeping
    file_handler = logging.FileHandler(log_file)
    file_handler.setLevel(logging.DEBUG)
    file_formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    file_handler.setFormatter(file_formatter)
    
    # Simpler logging to console for user-friendly experience
...
def is_tree_based_model(model):
    """Is this a tree-based model? (Decision Tree, Random Forest, etc.)"""
...
def is_linear_model(model):
    """Is this a linear model? (Linear regression, logistic regression, etc.)"""
    model_module = model.__module__
    model_name = model.__class__.__name__
    return any(name in model_module.lower() or name in model_name.lower() 
...
def is_neural_network(model):
    """Is this a neural network? (TensorFlow, PyTorch, etc.)"""
...
# Core functions for assessing explainability

def check_docstring_quality(function):
    """
    Rate the quality of a function's docstring - good docs are the first step to explainability!
    
    This checks if the docstring exists, has parameter descriptions, return value info, 
    examples, and gives suggestions for improvement.
    
    Returns a dictionary with quality metrics and suggestions.
    """
    ...
    # Check for key elements of a good docstring
    ...
    quality_score = 0
    if length > 0:
        # Up to 30 points for length - not too short, not too long
        quality_score += min(length / 5, 30)
    if has_parameters:
        # 25 points for documenting what goes in
        quality_score += 25
    if has_return_info:
        # 25 points for documenting what comes out
        quality_score += 25
    if has_examples:
        # 20 points for showing how to use it
        quality_score += 20
...
def analyze_function_complexity(function):
    """
    Analyze how complex a function is - complex functions are harder to understand!
    ...
    Returns a dictionary with complexity metrics and an interpretability assessment.
    """
    try:
        source_code = inspect.getsource(function)
    except (TypeError, OSError) as e:
        logger.warning(f"Couldn't get the source code: {e}")
        return {
    ...
    # Count lines of code (simple but effective metric)
    ...
    # How deeply nested is the code? (Using indentation as a proxy)
    ...
    # Calculate overall complexity score - higher means harder to understand
    # This is a weighted formula I've found works well in practice
    complexity_score = (
        conditional_counts * 2 +    # Conditionals add complexity
        loop_counts * 3 +           # Loops add more complexity
        nested_depth * 5 +          # Deep nesting adds a lot of complexity
        lines_of_code / 10          # Length adds some complexity
    )
    
    # How much branching logic per line of code?
    branching_factor = conditional_counts / lines_of_code if lines_of_code > 0 else 0
    
    # Classify interpretability
...
def extract_feature_importance(model, feature_names=None, X=None, y=None):
    """
    Extract feature importance from a model using the best available method.
    
    This tries several approaches, in order of preference:
    1. Built-in feature importance (like in tree models)
    2. Model coefficients (for linear models)
    3. Permutation importance (requires data)
    4. SHAP values (requires data)
    
    Returns a dictionary with feature importance info from all available methods.
    """
    importance_methods = []
    importance_values = {}
    error_messages = []
    
    # Method 1: Built-in feature importance (like in RandomForest)
    ...
    # Method 2: Model coefficients (for linear models)
    ...
    # Method 3: Permutation importance (model-agnostic but requires data)
    ...
    # Method 4: SHAP values (model-agnostic but requires data)
    if SHAP_AVAILABLE and X is not None:
        try:
            # Use a subset of data for efficiency with large datasets
...
def analyze_model_structure(model):
    """
    Analyze a model's structure to assess its inherent interpretability.
    
    Different model types have different levels of interpretability:
    - Linear models are usually most interpretable
    - Small decision trees are quite interpretable
    - Random forests and boosted trees are less interpretable
    - Deep neural networks are typically least interpretable
    
    Returns a dictionary with structure info and interpretability score.
    """
    model_type = "unknown"
    ...
    if is_tree_based_model(model):
        ...
        # Extract info about tree structure
        ...
        # Linear models are generally easy to interpret
        interpretability_score = 80
        
        # Check for sparsity - sparse models are more interpretable
        if hasattr(model, "coef_"):
            coefs = model.coef_
            ...
            # Sparse models are more interpretable (fewer coefficients to consider)
            if sparsity > 0.5:
        ...
        # Neural networks are generally hard to interpret
        ...
        # Unknown model type - we'll try to extract some basic info
        model_type = "unknown"
    ...
    # Convert the score to a category
...
def generate_lime_explanation(model, X, feature_names=None, class_names=None, instance_idx=0):
    ...
    LIME creates local surrogate models that approximate the behavior of the 
    black-box model around specific instances. It's great for explaining 
    individual predictions.
    
    Returns a dictionary with LIME explanation results for the specified instance.
    """
    if not LIME_AVAILABLE:
        return {"error": "LIME package not available. Install with 'pip install lime'."}
    ...
    try:
        ...
        if class_names is None:
            # If no class names provided, assume binary classification
            class_names = [0, 1]
        ...
        # Set up the prediction function based on the model type
        ...
        if mode == "classification":
            ...
            try:
                ...
                # Fallback to first class if there's an issue
                exp_data = explanation.as_list(label=0)
        else:
            # For regression
            exp_data = explanation.as_list()
        
        # Format the explanation data nicely
        lime_features = []
...
def generate_shap_explanation(model, X, feature_names=None, instance_idx=0):
    ...
    SHAP (SHapley Additive exPlanations) uses game theory to assign credit 
    to each feature. It's generally more theoretically sound than LIME but
    can be slower.
    
    Returns a dictionary with SHAP values for the specified instance.
    """
    if not SHAP_AVAILABLE:
        return {"error": "SHAP package not available. Install with 'pip install shap'."}
    ...
    try:
        ...
        # Choose the right explainer based on model type
        if is_tree_based_model(model):
            # Tree models have a specialized, fast SHAP explainer
            explainer = shap.TreeExplainer(model)
...
def evaluate_explanation_quality(explanation_data):
    """
    Evaluate how good an explanation is.
    
    A good explanation should be:
    - Complete (cover important aspects)
    - Consistent (not contradictory)
    - Compact (not too verbose)
    - Clear (understandable)
    
    Returns a dictionary with explanation quality metrics.
    """
    metrics = {
        "completeness": 0,
        "consistency": 0,
    ...
    if "error" in explanation_data:
        # If the explanation failed, score it as 0
    ...
    weights = {
        "completeness": 0.3,  # More complete explanations are better
        "consistency": 0.3,   # More consistent explanations are better
        "compactness": 0.2,   # More compact explanations are better
        "clarity": 0.2        # Clearer explanations are better
    }
...
def generate_decision_rules(model, feature_names=None):
    """
    Extract human-readable decision rules from a model when possible.
    
    Most effective for:
    - Decision trees (direct rule extraction)
    - Rule-based models
    - Some linear models (can be converted to rules)
    
    Returns a dictionary with decision rules and metadata.
    """
    rules = []
    
    # Only certain model types can generate rules
    ...
    try:
        if is_tree_based_model(model):
            # Extract rules from tree-based model
    ...
    return {
        "rules": rules[:10],  # Limit to top 10 rules for readability
...
def generate_visualizations(model, X, y=None, feature_names=None, output_dir="explainability_visualizations"):
    """
    Create visualizations to help understand the model.
    
    This creates various plots depending on the model type:
    - Feature importance plots
    - SHAP summary plots
    - Decision tree visualizations
    - Partial dependence plots
    
    Returns a dictionary with visualization information.
    """
    ...
    if SHAP_AVAILABLE and len(X_values) > 0:
        try:
            # Use a sample of the data for SHAP (for efficiency)
            ...
            if is_tree_based_model(model):
                ...
                background = shap.kmeans(X_values, 5)  # Use k-means for efficiency
...
def create_human_readable_explanation(model, X, y=None, feature_names=None, instance_idx=0):
    """
    Generate a human-friendly explanation of a prediction.
    
    This combines multiple techniques to create an explanation that:
    1. Identifies the key factors affecting the prediction
    2. Explains their influence in natural language
    3. Suggests counterfactual examples
    
    Returns a dictionary with the explanation components.
    """
    explanation = {}
    ...
    try:
        ...
        # Try to generate LIME explanation first
        if LIME_AVAILABLE:
        ...
        # Add counterfactual example (what would change the prediction?)
        if feature_values:
...
def generate_structured_report(results, model=None, function=None, output_file="explainability_report.json"):
    """
    Generate a structured JSON report with all explainability results.
    
    This collects all the analysis results and saves them in a standardized format
    that can be used for further processing, visualization, or documentation.
    
    Returns a dictionary with the full report.
    """
...
def generate_html_report(results, output_file="explainability_report.html"):
    """
    Generate a nice HTML report that's easy to share.
    
    This is more user-friendly than the JSON report and includes visualizations
    and styling to make it easy to interpret.
    
    Returns True if successful, False otherwise.
    """
    try:
        # Define HTML template with nice styling
        html_template = """
        <!DOCTYPE html>
        <html>
        <head>
...
def evaluate_model_explainability(model, X=None, y=None, feature_names=None, output_dir="explainability_results"):
    """
    Run a comprehensive explainability assessment for a model.
    
    This is the main entry point for model assessment, running all applicable
    analysis functions and generating reports.
...
def evaluate_function_explainability(function, output_dir="explainability_results"):
    """
    Run a comprehensive explainability assessment for a function.
    
    This is the main entry point for function assessment, running all applicable
    analysis functions and generating reports.
...
# My to-do list for further improvements
"""
TODO:
- Add support for more model types (ensemble methods, deep learning frameworks)
- Add benchmarking against "gold standard" human interpretations
- Add support for explainability of embeddings and representations
- Include more LIME and SHAP visualization options
- Implement better counterfactual explanation generation
- Add evaluation of explanation faithfulness and stability
- Support for multi-modal explanations (text + visualization)
- Add more natural language explanation templates
- Integrate with popular model registries
- Extend reporting for legal and compliance needs
"""
